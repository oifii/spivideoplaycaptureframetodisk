...
//------------------------------------------------------------------------------
// File: spivideoplaycaptureframetodisk.cpp
//
...
#include "spivideoplaycaptureframetodisk.h"
#include "resource.h"
...
MMRESULT global_timer_capture=1;
DWORD global_startstamp_ms;

WCHAR global_TempBuffer[1024];
int global_imageframe_id=0;
int global_imageframe_max=1000; //1000 images, will loop and overwrite over the 1000 frames
DWORD global_imageframe_capturerate_ms = 500;
WCHAR global_imageframe_path[] = {L"c:\\temp\\spivideocaptureframetodisk\\"}; //must terminate by \\
...
//spi, begin
void CALLBACK CaptureFrameProcess(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
	PostMessage(global_hwnd, WM_COMMAND, ID_CAPTURE_IMAGE, 0);	
}
                ...
                case ID_CAPTURE_IMAGE:
					{
						...
						//wsprintf(global_TempBuffer, L"c:\\temp\\spivideocaptureframetodisk\\frame%d.bmp", global_imageframe_id);
						_wmkdir(global_imageframe_path);
						std::wstring mywstring;
						mywstring = global_imageframe_path;
						wsprintf(global_TempBuffer, L"frame%03d.bmp", global_imageframe_id);
						mywstring += global_TempBuffer;
						wcscpy(global_TempBuffer, mywstring.c_str());
						//CaptureImage(CAPTURED_IMAGE_NAME);
						CaptureImage(global_TempBuffer);
					}
                    break;
...
int PASCAL wWinMain(HINSTANCE hInstC, HINSTANCE hInstP, LPWSTR lpCmdLine, int nCmdShow)
{
	...
	*/
    ...
    if(ghApp)
    {
		...
		//global_timer_capture=timeSetEvent(global_imageframe_capturerate_ms,25,(LPTIMECALLBACK)&CaptureFrameProcess,0,TIME_PERIODIC);
		//spi, end
